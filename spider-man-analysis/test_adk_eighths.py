...
# Load environment variables from sd1/.env
env_path = Path(__file__).parent.parent / "sd1" / ".env"
load_dotenv(env_path)

# Import both implementations
from sd1.src.script_ingestion.agents.eighths_calculator_agent import EighthsCalculatorAgent
    ...
    def extract_scenes_from_pdf(self) -> List[Dict[str, Any]]:
        ...
        try:
            with open(self.pdf_path, 'rb') as file:
                ...
                # Process each page
                for page_num in range(min(total_pages, 10)):  # Limit to first 10 pages for testing
                    ...
                    # Store page-level eighths data
                    page_eighths = {
                        "page_number": page_num + 1,
                        "word_count": len(text.split()),
                        "estimated_eighths": (len(text.split()) / 250) * 8,
                        "scenes_on_page": []
                    }
